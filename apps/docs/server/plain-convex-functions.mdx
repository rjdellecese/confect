...
Run `confect codegen` to assemble leaf specs into `confect/_generated/spec.ts`.
...
Run `confect codegen` to emit the per-group registry that wires this leaf impl into `convex/`.
