...
Query streams let you merge, filter, join, and deduplicate index queries while retaining cursor pagination. A **query stream** is an Effect [`Stream`](https://effect.website/docs/stream/introduction) of decoded documents with a key layout and direction. Use `reader.table(...).stream(...)` for standard indexes; search indexes use [`search`](/server/database/reading#search-indexes).
...
| Operation                                                                 | Key layout                                     | In Effect                                                        |
| ------------------------------------------------------------------------- | ---------------------------------------------- | ---------------------------------------------------------------- |
...
| [`flatMap`](#flat-map)                                                    | Extended by the inner layout                   | Sequential `Stream.flatMap`                                      |
| [`flatMap` with `onEmpty`](#keep-outer-documents-without-inner-documents) | Extended by the inner layout                   | `Stream.orElseIfEmpty` on each inner stream                      |
| [`renameKey`](#rename-ordering-labels)                                    | Visible labels replaced                        | No equivalent                                                    |
...
The `Labels` and `Direction` type parameters enforce this contract: `merge` refuses streams whose known labels or directions differ at compile time. Runtime checks also require matching directions, visible labels, and implicit-ID positions.
...
`notes` has the indexes `by_text` on `["text"]` and `by_role` on `["author.role"]`; `comments` has `by_note` on `["noteId"]` and `by_body` on `["body"]`. Runtime key values include `_creationTime` and `_id` tiebreakers, so `by_text` orders by text, creation time, then ID.
...
A key contains the values that determine a document's position. For a note ordered by text, it might be `["apple", 1, "n1"]`: text, creation time, and an ID tiebreaker. Pinning text with `eq` leaves `[1, "n1"]`.

**Labels** name the visible ordering positions, such as `["text", "_creationTime"]`. They start as index field paths, but `renameKey` can replace them with aliases. Pass these labels to `distinct` and `renameKey`; implicit ID tiebreakers are retained automatically.

A stream's `keyLayout` describes its ordering, including those tiebreakers. Reuse it when calling `empty` or supplying `flatMap`'s `innerLayout`. For explicit `narrow` bounds, use key **values** instead of labels. A joined key includes both the outer and inner document IDs, keeping documents with equal timestamps distinct.

### Empty streams

`QueryStream.empty` is a query stream with no documents but a known key layout, for the places where a stream is required and there is nothing to put in it. Its usual job is a merge over a list of streams that may turn out empty, since `merge` needs at least one input.
...
Once you apply a plain `Stream` combinator the result is an ordinary `Stream`: it can be consumed, but it no longer retains its key layout, so it can't be merged or paginated further. Use the `QueryStream` combinators below when you need to keep that ability.
...
`QueryStream.merge` interleaves streams that share a key layout into one ordered stream. Merging is how you query over several index ranges at once, such as the notes by two roles in the example at the top of this page.
...
Pinning `author.role` leaves both streams with the visible labels `["_creationTime"]`, so the merge interleaves them by creation time:
...
Inputs must have compatible document types and identical key layouts and directions. TypeScript rejects known mismatches; runtime mismatches throw when the streams are combined. For different document shapes or field names, [map and relabel](#rename-ordering-labels) first. Overlapping inputs are not deduplicated: a document matched by two inputs appears twice.

### Filter and map

`QueryStream.filter` removes emitted values; `QueryStream.map` transforms them. Both preserve the original stored key values. A rejected document still counts as _read_, so cursors can advance past it.
...
`QueryStream.distinct` keeps the first document for each distinct value of a key prefix.
...
The labels must be a prefix of the stream's visible labels, which is enforced at the type level. On a `flatMap` result, a prefix that reaches into the inner key groups by the outer document as well, since groups are runs of consecutive equal keys.
...
`QueryStream.narrow` restricts a stream to the key values between `start` and `end`. Each endpoint has `keyValues` and a required `inclusive` flag, so you can choose any combination of inclusive and exclusive bounds. Provide at least one endpoint; omit the other to leave that side unbounded.
...
Every inner stream must have the ordering described by `innerLayout` and run in the outer stream's direction. Here, pinning `noteId` leaves comments ordered by creation time, so a creation-time stream supplies a compatible layout. TypeScript rejects known mismatches; others fail when the join runs.

The joined key keeps both IDs: comment `c1` has key `[1, "n1", 7, "c1"]`. For nested joins or renamed inner streams, reuse the `keyLayout` of a compatible composed stream.
...
`QueryStream.renameKey` relabels a stream's visible ordering positions. It does not sort: the elements and their order are untouched.
...
| `Labels`             | A branded readonly tuple of visible ordering labels, such as `["text", "_creationTime"]`. Inferred from the index and updated by `eq`, `flatMap`, and `renameKey`; implicit ID tiebreakers are omitted. |
