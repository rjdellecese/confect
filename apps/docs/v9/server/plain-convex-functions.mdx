...
Write standard Convex functions using the Convex API and place them in your `confect/` directory. By [convention](/v9/concepts/file-naming-conventions), they go adjacent to their corresponding spec and impl files—for example, `workpool.ts` alongside `workpool.spec.ts` and `workpool.impl.ts`. The component reference comes from Confect's generated [components registry](/v9/server/components#referencing-components).
