...
Each function spec defines the function's name, arguments schema, and returns schema. It can also declare an optional `error` schema to expose [typed errors](/server/typed-errors) to callers. Function specs are grouped into group specs, which are combined into your API's top-level spec. See [The Spec/Impl Model](/concepts/spec-impl-model) for a full walkthrough.
