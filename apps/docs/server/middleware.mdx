...
Equality follows the options schema, so array order matters by default. Use `Schema.overrideToEquivalence` on the options schema if your policy needs custom equality semantics.
