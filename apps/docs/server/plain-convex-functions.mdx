...
Write standard Convex functions using the Convex API. Place them in your `confect/` directory — a common convention is a `plain/` subfolder.

```ts confect/plain/workpool.ts
import {
  type WorkId,
  Workpool,
  vWorkId,
...
import type { search } from "../plain/search";
