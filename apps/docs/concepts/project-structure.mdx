...
Use `DatabaseSchema.tables(databaseSchema)` to inspect the bound table record. Reading this record does not evaluate the tables' schemas. A handwritten type annotation takes the record shape, for example `DatabaseSchema.DatabaseSchema<{ readonly notes: typeof notes }>`. `DatabaseSchema.Tables<typeof databaseSchema>` derives the union of its table types.
...
When inspecting the assembled spec in tooling or tests, use `Spec.groups(spec)` to access its group record. `Spec.Spec` takes that record's type, and `Spec.Groups<typeof spec>` derives its member union. The immutable `add` and `addAt` builders infer this record automatically; adding a group at an existing name replaces the previous group's value and type.
