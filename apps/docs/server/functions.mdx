...
Each function spec defines the function's name, arguments schema, and returns schema. Function specs are added to a `GroupSpec` and default-exported from a `*.spec.ts` file. See [The Spec/Impl Model](/concepts/spec-impl-model) for a full walkthrough.
...
Run `confect codegen` after adding or changing impls.
