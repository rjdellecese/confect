...
Retrieve a page of documents matching the query. Expects a [`PaginationOptions`](https://docs.convex.dev/api/interfaces/server.PaginationOptions) object, including Convex's optional pagination protocol fields. A [paginated query](/v10/server/functions#paginated-queries) handler typically forwards its decoded `paginationOpts` argument unchanged.

```ts
reader.table("notes").index("by_creation_time").paginate(paginationOpts);
```
...
To merge, join, or paginate several queries together, use a [query stream](/v10/server/database/streams) instead—`reader.table("notes").stream("by_creation_time")`—which stays paginable after composition.

### Accessing the current time

Reading real time inside a query handler affects how aggressively Convex caches the result. See [Determinism](/v10/server/database/determinism) for how Confect keeps queries cacheable by default and how to opt in to real time via Effect's `Clock` service.
