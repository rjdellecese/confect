...
Each method returns an `Effect` that can fail with `HttpClientError` (wrapping transport-level errors), `ParseResult.ParseError` (if schema encoding or decoding fails), or the ref's declared [typed error](/server/error-handling), if any:
