...
`paginate` uses an exclusive start and an inclusive end. For a nonterminal page, decode its `continueCursor` with `QueryStreamCursor.deserialize(page.continueCursor, query.keyFields)` and pass that full runtime key as an exclusive `start` to `narrow`. Do not decode `QueryStreamCursor.END_CURSOR` as a boundary; it is an end-of-stream sentinel.
...
The `QueryStreamCursor` module, exported from `@confect/server`, owns the envelope schema and inferred `QueryStreamCursor` type. Its `Json` codec encodes that envelope as a string; its `OrderKey` type describes decoded Convex values, including missing fields as `undefined`.

| `QueryStreamCursor` export        | Contract                                                                                                                                                             |
| --------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `serialize(key, keyFields)`       | Serializes a full runtime `OrderKey` with its runtime field names. Both arguments are required; use the stream's `keyFields`, not its type-level `Key`.              |
| `deserialize(cursor, keyFields?)` | Validates the envelope and returns a decoded runtime `OrderKey`, preserving IDs. If expected `keyFields` are supplied, also validates the exact ordered field names. |
