...
There is no `HttpApiBuilder.api` layer to wrap the group layers in — Confect registers the routes itself from the `api` definition — so export the group layer(s) directly (merge multiple groups with `Layer.mergeAll`).
