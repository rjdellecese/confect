...
<Info>
  `_generated/nodeSpec.ts` is only generated when you have node action
  specs (files under `confect/node/`).
</Info>
...
Contains your assembled Node spec. It is merged into `refs.ts` and referenced type-only by the node groups' `registeredFunctions/` modules.
...
Contains your assembled Confect spec. It is used to build `refs.ts` and is referenced type-only by the `registeredFunctions/` modules.
