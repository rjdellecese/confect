...
`@gunta/confect-test` wraps [`convex-test`](https://docs.convex.dev/testing) with Effect-native APIs. It provides a `TestConfect` service that lets you call your Confect functions using refs, run arbitrary mutation-context code for test setup, and test with user identities.
...
```bash pnpm
pnpm add -D @gunta/confect-test convex-test @effect/vitest
```

```bash npm
npm install -D @gunta/confect-test convex-test @effect/vitest
```

```bash yarn
yarn add -D @gunta/confect-test convex-test @effect/vitest
```

```bash bun
bun add -D @gunta/confect-test convex-test @effect/vitest
```
