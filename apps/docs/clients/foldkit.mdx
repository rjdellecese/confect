...
Command needs a custom args schema, [interruption](https://foldkit.dev/api-reference/command),
or several calls in one Command:
...
Paginated queries (`usePaginatedQuery`'s Foldkit counterpart) and `interrupt`
support on the Command factories are not yet built in. Both have escape
hatches: paginated refs can be called through the
[`WebSocketClient`](/clients/js/websocket) service directly, and interruption
composes through the [effect helpers](#effect-helpers) as shown above.
