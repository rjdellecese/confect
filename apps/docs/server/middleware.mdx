...
Equality follows the options schema. Arrays are ordered by default, so `["admin", "editor"]` and `["editor", "admin"]` are distinct. Use `Schema.overrideToEquivalence` to customize equality when your policy treats them as the same set. Function behavior cannot be compared: resolver functions need reference equality or a custom equivalence. Prefer named resolver IDs when you need reliable duplicate detection.
