...
Declare an `options` type in a middleware's `Config` to reuse one policy with different attachment values. For example, a resource gate can take the name of the argument containing its id:
...
export default class RequireResource extends MiddlewareSpec.MiddlewareSpec<
  RequireResource,
  { options: { readonly idArgument: string } }
>()("RequireResource", {
...
The second argument is required and checked against the declared type. Middleware without an `options` declaration still uses `.middleware(Spec)` with no second argument. The declaration is type-level, like `provides` and `requires`; attachment values are not decoded through a runtime schema.
...
Options belong to each attachment, so different functions can reuse the same middleware implementation with different values. They survive codegen and are carried by `refs`; the middleware's declared error still determines the client error union, independently of its options. `provides` and `requires` also remain fixed for a spec: an option does not change a handler's service types. For a `tolerateMissing` policy, declare a service containing an `Option` and let the implementation decide whether to provide `None` or fail.

<Warning>
  Attachment values are reachable from client refs. Keep them and any resolver
  functions client-safe: do not put secrets, server-only imports, or privileged
  lookup logic in options. Keep authorization and database reads in the impl.
</Warning>

Duplicate detection remains by middleware key: attaching the same spec twice is a type error and a runtime error, even with different options. This includes one attachment on a group and another on one of its functions. Attachment order, function-type restrictions, and subgroup boundaries are unchanged.
