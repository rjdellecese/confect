...
The `Labels` and `Direction` type parameters enforce this contract: `merge` refuses streams whose known keys or directions differ at compile time. Runtime checks also require matching directions, visible labels, and implicit-ID positions.
...
The type parameter `Labels` tracks the visible labels of a stream's ordering. At index construction, these labels come from the source field paths that remain after equality pinning, including `_creationTime`. `reader.table("notes").stream("by_text")` starts with labels `["text", "_creationTime"]`; its `keyLayout` also records an implicit ID, so an element's runtime key can be `["apple", 1, "n1"]`. Pinning text with `eq` removes its label and value from the stream key.

Pass visible ordering labels to `distinct` and replacement labels to `renameKey`. Pass a typed `keyLayout` to `empty` and to `flatMap` as `innerLayout`: it records the visible labels and every implicit ID. Use runtime key values for exact `narrow` boundaries and cursor serialization. Preserve every `_id` in a boundary: it distinguishes documents with otherwise equal keys. A [flat-map](#flat-map) keeps an ID for each level of the join, not just the final document.

A layout is a sequence of component keys. Each component has visible labels and may end in an implicit ID. A join concatenates components; renaming changes only visible labels. For example, `QueryStreamKeyLayout.fromIndex(["_id"], 1)` succeeds with a layout describing a fully pinned `by_id` scan with no runtime positions, while the layout returned by `QueryStreamKeyLayout.fromIndex(["text", "_creationTime"], 2)` retains one implicit ID. Both have the visible labels `[]`, so visible labels alone cannot identify a layout. Renaming changes these labels into aliases; they need not identify properties of the emitted document. Index extraction and range construction continue to use the original source field paths.

`QueryStreamKeyLabels` represents the ordered names of visible key components. It preserves duplicates and can be empty. `QueryStreamKeyLayout.visibleLabels(layout)` returns this value, retaining the layout's literal tuple type. Use `QueryStreamKeyLabels.toArray(labels)` to inspect the names.

`distinct` and `renameKey` accept literal tuples directly. When working with layouts themselves, construct a labels value explicitly:

```ts
import { QueryStreamKeyLabels, QueryStreamKeyLayout } from "@confect/server";
import * as Result from "effect/Result";

const layout = Result.getOrThrow(
  QueryStreamKeyLayout.fromIndex(["authorId", "_creationTime"]),
);
const renamed = Result.getOrThrow(
  QueryStreamKeyLayout.rename(
    layout,
    QueryStreamKeyLabels.make(["author", "created"]),
  ),
);
...
`fromIndex`, `resolvePrefix`, and `rename` return `Result` values. Their failures are `InvalidEqualityPrefixError`, `InvalidLabelPrefixError`, and `LabelCountMismatchError`, respectively, with structured inputs available for inspection. Handle the result directly, or yield it inside `Effect.gen` to retain the typed failure channel. These examples use `Result.getOrThrow` for fixed, known-valid layouts. Stream combinators treat invalid arguments as API misuse and throw the named error; failures discovered while running a stream become defects.

Labels provide sequence equality, concatenation, and prefix parsing. A layout additionally records the positions of implicit IDs, so matching visible labels alone does not establish layout compatibility.

Use `QueryStreamKeyLayout.segments(layout)` to inspect the component segments. Each segment holds a labels value and records whether it ends in an implicit ID.
...
Pass the document type as a type argument, then the layout of the streams it will be merged with. Reuse an existing stream's `keyLayout`, or construct a scan layout with `QueryStreamKeyLayout.fromIndex`. The [generated document types](/server/database/schema#document-types) name the document:

```ts
import { QueryStreamKeyLayout } from "@confect/server";
...
const innerLayout = Result.getOrThrow(
  QueryStreamKeyLayout.fromIndex(["noteId", "_creationTime"], 1),
);
...
Pass the inner streams' layout as `innerLayout`; its logical labels are checked against the type of the stream the function returns. Every returned stream is also checked for matching visible labels and implicit-ID positions. Inner streams must run in the outer stream's direction: with `QueryStream.flatMap(outer, f, options)` the outer stream fixes it and a differing inner stream is a type error, while with `outer.pipe(QueryStream.flatMap(f, options))` the inner streams fix it. A mismatch the types can't see fails when the join runs.

Here the visible ordering labels are `["_creationTime", "_creationTime"]`, but the runtime layout is `["_creationTime", "_id", "_creationTime", "_id"]`: the outer note's complete key, then the comment's. For `c1`, that is `[1, "n1", 7, "c1"]`. Keep both IDs when serializing or narrowing a joined stream; equal outer timestamps must not collapse distinct notes into one group.

An inner stream can itself be a join. Reuse its `keyLayout`, or compose the layouts of its outer and inner streams with `QueryStreamKeyLayout.concat`. This preserves the intermediate IDs that a list of logical names omits:

```ts
const nestedInnerLayout = QueryStreamKeyLayout.concat(
  commentsLayout,
  reactionsLayout,
);

const joined = QueryStream.flatMap(notes, commentsWithReactions, {
  innerLayout: nestedInnerLayout,
});
const nothing = QueryStream.empty<CommentWithReaction>()(joined.keyLayout);
```

Supply the layout before running the join so an empty inner stream can still contribute the correctly sized cursor marker. A renamed inner stream must supply its renamed layout as well.
...
Boundary cursors are JSON strings containing a versioned envelope: `{ version: 1, keyFields: [...], orderKey: [...] }`. `keyFields` contains the serialized ordering labels for every runtime position, including implicit IDs; `orderKey` contains the corresponding encoded values. For the example note `n3`, the serialized cursor string contains this JSON object (using the example's stand-in ID):
...
| -------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `QueryStreamCursor`        | Schema and inferred type for the full native cursor: version, serialized runtime labels, and key values.                                                                                                       |
| `Json`                     | String codec for the full native cursor, including Convex's special-value encodings.                                                                                                                           |
| `codecForLayout(layout)`   | Returns a string codec for a `QueryStreamOrderKey`, bound to a structured layout. Encoding derives the runtime labels, including implicit IDs; decoding checks those labels in order. Pass `stream.keyLayout`. |
...
Effect.gen(function* () {
  const Cursor = QueryStreamCursor.codecForLayout(query.keyLayout);
...
`paginate` validates boundary cursors against the stream's runtime layout. Malformed envelopes, unsupported versions, and mismatched serialized ordering labels fail with a `ConvexError` whose data is `{ paginationError: "InvalidCursor" }`. Confect's pagination clients treat that signal as a request to restart. The string `"[]"` is accepted only as an `endCursor` sentinel.

The envelope preserves its existing version-1 format. It does **not** identify the query, index, filters, pinned `eq` values, direction, segment boundaries, or whether an `"_id"` label was explicit or implicit. A change to any of those that preserves the serialized ordering labels is not automatically detected. Restart pagination when changing query semantics; do not rely on cursor validation to invalidate every cursor after a deployment.
...
| `Labels`             | Ordered field-name tuple left after `eq` pins fields, such as `["text", "_creationTime"]`. Omits implicit `_id` fields; see [Order keys](#order-keys) for the runtime layout. |
...
A `QueryStream<Doc, Labels, Direction, E, R>` is also a `Stream<Doc, E, R>`; only `QueryStream` carries the key and direction types.
