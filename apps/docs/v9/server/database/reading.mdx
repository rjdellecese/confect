...
Reading real time inside a query handler affects how aggressively Convex caches the result. See [Determinism](/v9/server/database/determinism) for how Confect keeps queries cacheable by default and how to opt in to real time via Effect's `Clock` service.
