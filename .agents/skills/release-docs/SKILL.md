...
Deploy the docs on `main`, or the ref requested in the task, by triggering the
"Docs Release" workflow (`.github/workflows/docs-release.yml`). The workflow
force-pushes that ref to the `release` branch. Mintlify deploys docs from that
branch, so this publishes docs updates without cutting a new npm release.
