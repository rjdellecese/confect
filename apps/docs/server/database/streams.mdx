...
It runs the stream narrowed to the keys after `cursor` (and up to `endCursor`, when given), collects `numItems` documents that pass the filters into a page, and reports a safe logical key as the next cursor. With `endCursor`, it reads through that cursor inclusively and ignores `numItems`. A cursor is a key rather than an offset, so resuming does not require scanning all earlier results, though a composition may need extra reads to produce each item.
