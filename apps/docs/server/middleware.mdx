...
A middleware declares which function types it may cover with the optional `functionTypes` option — any subset of `"query"`, `"mutation"`, and `"action"` (Node actions count as `"action"`). The default is all three. Attaching a middleware to a group is a type error unless every function's type is among the middleware's `functionTypes`, so the `RequireUser` above only fits groups of queries and mutations, and a `functionTypes: ["mutation"]` middleware only fits all-mutation groups.
...
| Declared `functionTypes`                   | Services available to `make`                                                           |
| ------------------------------------------ | -------------------------------------------------------------------------------------- |
