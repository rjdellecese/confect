...
<Note>
  Confect emits one Convex module per group, so a function's
  cold-start bundle only includes its own group's spec, impl, and the
  tables it touches. To keep that bundle small, import Effect from its
  submodule paths (`import * as Schema from "effect/Schema"`) rather
  than the `effect` barrel in your `confect/` files—the barrel defeats
  tree-shaking and re-pins the whole `Schema` namespace into every
