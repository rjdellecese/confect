...
Declare a lazy `options` schema in the middleware constructor to reuse one policy with different attachment values. The schema's `Type` determines the options type; `Config` only declares `provides` and `requires`. For example, a resource gate can take the name of the argument containing its id:

```ts confect/middleware/RequireResource.spec.ts
import { MiddlewareSpec } from "@confect/core";
import * as Schema from "effect/Schema";
...
export default class RequireResource extends MiddlewareSpec.MiddlewareSpec<RequireResource>()(
  "RequireResource",
  {
    options: () => Schema.Struct({ idArgument: Schema.String }),
...
Pass the options as the second argument to `.middleware()`, on either a function or a group:

```ts
FunctionSpec.publicQuery({
  name: "getResource",
  args: () => ({ resourceId: Schema.String }),
  returns: () => Schema.String,
}).middleware(RequireResource, { idArgument: "resourceId" });

GroupSpec.make().middleware(RequireResource, { idArgument: "id" });
```
...
Keep the schema inside its thunk. Constructing middleware specs and assembling groups, specs, and refs does not evaluate options schemas; validation happens later.

Both `MiddlewareImpl.make` and `MiddlewareImpl.makeByFunctionType` receive the attachment value in the invocation metadata's typed `options` field, alongside `name`, `functionType`, `functionVisibility`, and decoded `args`. Middleware without options receives `undefined` in that field:

```ts confect/middleware/RequireResource.impl.ts
import { MiddlewareImpl } from "@confect/server";
import * as Effect from "effect/Effect";
import * as Predicate from "effect/Predicate";

import databaseSchema from "../_generated/schema";
import RequireResource, { MissingResourceId } from "./RequireResource.spec";

export default MiddlewareImpl.make(
  databaseSchema,
  RequireResource,
  (effect, { args, options }) =>
    Predicate.hasProperty(args, options.idArgument)
      ? effect
      : Effect.fail(new MissingResourceId()),
);
```

This example only checks that the argument exists; an authorization gate would also check access to the referenced resource. The same options mechanism supports role lists, flags, and client-safe resolver functions. Use `MiddlewareImpl.make` rather than the `provides` shorthand when producing a service depends on the attachment options.

Options belong to each attachment, so different functions can reuse the same middleware implementation with different values. They survive codegen and are carried by `refs`. To inspect attachments, use the ordered `middlewareAttachments` list of `{ spec, options }` entries; `middlewareSpecs` remains a specs-only view for compatibility, not a source of per-attachment options.
...
```ts
GroupSpec.make()
  .middleware(RequireResource, { idArgument: "sourceId" })
  .addFunction(
    FunctionSpec.publicMutation({
      name: "copy",
      args: () => ({ sourceId: Schema.String, destinationId: Schema.String }),
      returns: () => Schema.Null,
    }).middleware(RequireResource, { idArgument: "destinationId" }),
  );
```

Both checks must pass before the handler runs. Repeated guards form a conjunction, not a union of allowed policies: two role guards mean the caller must satisfy both, not either. An earlier short-circuit still prevents the remaining attachments and handler from running.

Confect uses `Schema.toEquivalence` to compare options for attachments with the same middleware key. Equivalent options are rejected by `confect codegen` and by server registration, including across the group/function boundary. TypeScript does not check this equivalence. Middleware without an options schema still rejects duplicate keys immediately, both at the type level and at runtime.
...
### Customizing option equality

Equality follows the options schema. Arrays are ordered by default, so `["Internal", "Buyer"]` and `["Buyer", "Internal"]` are distinct. If your role gate treats its allowed roles as a set, use `Schema.overrideToEquivalence` so order and repeated entries do not make the same policy look different:
...
const roleOptions = () =>
  Schema.Struct({
    roles: Schema.Array(Schema.Literals(["Internal", "Buyer", "BrandPartner"])),
  }).pipe(
...
Pass this factory as `options: roleOptions` in the middleware declaration, without calling it. The schema stays lazy, and attachment values are inferred as arrays of the three allowed role literals. With this equivalence, attaching both `{ roles: ["Internal", "Buyer"] }` and `{ roles: ["Buyer", "Internal"] }` to the same chain is a duplicate, rejected during codegen or registration.

The equivalence must describe the whole policy configuration. If you add an option such as `tolerateMissing`, compare it too; otherwise configurations that behave differently could be rejected as duplicates. Keep the comparison pure, symmetric, and transitive.

Function behavior cannot be compared: resolver functions need reference equality or a custom equivalence. Prefer named resolver IDs when you need reliable duplicate detection.
...
export default class RequireAdmin extends MiddlewareSpec.MiddlewareSpec<
  RequireAdmin,
  {
...
A middleware implementation wraps the downstream effect (any remaining middleware plus the handler). It receives that effect together with metadata about the invocation — the covered function's `name`, `functionType`, and `functionVisibility`, plus its decoded `args` and attachment `options` — and decides whether and how to run it:
...
class Timed extends MiddlewareSpec.MiddlewareSpec<Timed>()("Timed", {
  functionTypes: { query: false, mutation: true, action: true },
}) {}
