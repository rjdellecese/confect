...
| Queries and mutations                      | `DatabaseReader`, `Auth`, `StorageReader`, `QueryRunner`, `ExecutionMetadata`, `TransactionMetadata`                           |
| Mutations and actions                      | `Auth`, `Scheduler`, `StorageReader`, `StorageWriter`, `QueryRunner`, `MutationRunner`, `ExecutionMetadata`, `RequestMetadata` |
| Any set including both queries and actions | `Auth`, `StorageReader`, `QueryRunner`, `ExecutionMetadata`                                                                    |

The [metadata services](/server/metadata) follow the same intersection rule. You can inspect the current function and deployment in any middleware, but request metadata requires mutations or actions, and transaction metrics require queries or mutations.
