...
import { FunctionImpl } from "@gunta/confect-server";
...
The `ConvexConfigProvider` module is exported from `@gunta/confect-server` if you need to create a provider with custom options. It supports the same options as Effect's default [`ConfigProvider.fromEnv`](https://effect.website/docs/configuration/config-provider/#fromenv).

```ts
import { ConvexConfigProvider } from "@gunta/confect-server";
