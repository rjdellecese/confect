...
Returns a `PaginatedQueryResult<Item, E>`. The loaded variants — `LoadingFirstPage`, `LoadingMore`, `CanLoadMore`, and `Exhausted` — all carry `results`, `isLoading`, and `loadMore`, so the common UI needs only field access and a couple of predicates:
...
`PaginatedQueryResult` also provides `match` for exhaustive pattern matching, and the predicates `isLoadingFirstPage`, `isLoadingMore`, `isExhausted`, `isLoading`, `isLoaded`, and `isFailure`.
...
// notes: Loaded — isLoading and loadMore are available too.
```

Every variant carries `results`, `Failure` included: when a later page fails, the pages already loaded are not discarded. `Failure` omits `loadMore`, which cannot make progress from a failed state.
