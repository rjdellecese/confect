...
describe("isConfectSchema", () => {
  ...
  test("is a type guard for DatabaseSchema.Any", () => {
    ...
    if (DatabaseSchema.isSchema(maybeSchema)) {
      expectTypeOf(maybeSchema).toExtend<DatabaseSchema.Any>();
    }
  });
});
...
describe("DatabaseSchema.Any", () => {
  ...
  test("ConfectSchema extends Any", () => {
    expectTypeOf<TestConfectSchema>().toExtend<DatabaseSchema.Any>();
  });

  test("empty schema extends Any", () => {
    const _emptySchema = DatabaseSchema.make();
    expectTypeOf<typeof _emptySchema>().toExtend<DatabaseSchema.Any>();
  });
});

describe("DatabaseSchema.AnyWithProps", () => {
  test("extends Any", () => {
    expectTypeOf<DatabaseSchema.AnyWithProps>().toExtend<DatabaseSchema.Any>();
  });

  test("has tables property as Record", () => {
    type Tables = DatabaseSchema.AnyWithProps["tables"];
    expectTypeOf<Tables>().toExtend<Record<string, Table.AnyWithProps>>();
  });

  test("has convexSchemaDefinition property", () => {
    type ConvexSchemaDef =
      DatabaseSchema.AnyWithProps["convexSchemaDefinition"];
  ...
  test("has addTable method", () => {
    type AddTable = DatabaseSchema.AnyWithProps["addTable"];
    expectTypeOf<AddTable>().toBeFunction();
  });

  test("ConfectSchema extends AnyWithProps", () => {
    expectTypeOf<TestConfectSchema>().toExtend<DatabaseSchema.AnyWithProps>();
  });
});

describe("DatabaseSchema.Tables", () => {
  test("extracts tables union from schema", () => {
    type Tables = DatabaseSchema.Tables<TestConfectSchema>;
  ...
  test("single table schema returns that table", () => {
    ...
    type Tables = DatabaseSchema.Tables<typeof _singleTableSchema>;
...
describe("DatabaseSchema.TableNames", () => {
  test("extracts table names as union of string literals", () => {
    type TableNames = DatabaseSchema.TableNames<TestConfectSchema>;
  ...
  test("single table schema has single name", () => {
    const _singleTableSchema = DatabaseSchema.make().addTable(notesTable);
    type TableNames = DatabaseSchema.TableNames<typeof _singleTableSchema>;
...
describe("DatabaseSchema.TableWithName", () => {
  test("returns the correct table for notes", () => {
    type Table = DatabaseSchema.TableWithName<TestConfectSchema, "notes">;
  ...
  test("returns the correct table for users", () => {
    type Table = DatabaseSchema.TableWithName<TestConfectSchema, "users">;
  ...
  test("preserves table indexes", () => {
    type NotesTableFromSchema = DatabaseSchema.TableWithName<
      TestConfectSchema,
      "notes"
    >;
...
describe("make", () => {
  ...
  test("tables type is never for empty schema", () => {
    const _schema = DatabaseSchema.make();
    type Tables = DatabaseSchema.Tables<typeof _schema>;
...
describe("systemSchema", () => {
  ...
  test("has correct table names", () => {
    type TableNames = DatabaseSchema.TableNames<
...
describe("extendWithSystemTables", () => {
  test("return type is a record with original table names as keys", () => {
    ...
    const tablesRecord: Table.TablesRecord<TestTables> = {
      notes: notesTable,
      users: usersTable,
    };
  ...
  test("return type has correct table types at each key", () => {
    ...
    expectTypeOf<(typeof _extended)["_scheduled_functions"]>().toEqualTypeOf<
      typeof Table.scheduledFunctionsTable
    >();
    expectTypeOf<(typeof _extended)["_storage"]>().toEqualTypeOf<
      typeof Table.storageTable
    >();
  });
  ...
  test("allows type-safe property access for system tables", () => {
    ...
    expect(extended._scheduled_functions).toBe(Table.scheduledFunctionsTable);
    expect(extended._storage).toBe(Table.storageTable);
  });
  ...
  test("return type extends TablesRecord of extended tables union", () => {
    type TestTables = NotesTable | UsersTable;
    type ExtendedTables = TestTables | Table.SystemTables;
    ...
    expectTypeOf(extended).toExtend<Table.TablesRecord<ExtendedTables>>();
  });
});
...
describe("ExtendWithSystemTables", () => {
  ...
  test("is equivalent to TablesRecord of extended tables union", () => {
    type TestTables = NotesTable | UsersTable;
    type ExtendedTablesUnion = TestTables | Table.SystemTables;
    type Extended = DatabaseSchema.ExtendWithSystemTables<TestTables>;

    expectTypeOf<Extended>().toEqualTypeOf<
      Table.TablesRecord<ExtendedTablesUnion>
    >();
  });
...
describe("Edge cases", () => {
  describe("schema with table with optional fields", () => {
    ...
    const schemaWithOptional =
      DatabaseSchema.make().addTable(tableWithOptional);
    ...
    test("has correct table names", () => {
      type TableNames = DatabaseSchema.TableNames<typeof schemaWithOptional>;
  ...
  describe("schema with table with multiple indexes", () => {
    const tableWithIndexes = Table.make(
      "indexed",
      Schema.Struct({
    ...
    test("preserves indexes in schema table", () => {
      type Table = DatabaseSchema.TableWithName<
  ...
  describe("schema with many tables", () => {
    const table1 = Table.make("table1", Schema.Struct({ a: Schema.String }));
    const table2 = Table.make("table2", Schema.Struct({ b: Schema.Number }));
    const table3 = Table.make("table3", Schema.Struct({ c: Schema.Boolean }));
    const table4 = Table.make(
      "table4",
      Schema.Struct({ d: Schema.Array(Schema.String) }),
    );
    ...
    test("contains all tables", () => {
      type TableNames = DatabaseSchema.TableNames<typeof largeSchema>;
      expectTypeOf<TableNames>().toEqualTypeOf<
  ...
  describe("empty schema behavior", () => {
    ...
    test("TableNames is never", () => {
      type TableNames = DatabaseSchema.TableNames<typeof emptySchema>;
      expectTypeOf<TableNames>().toEqualTypeOf<never>();
    });

    test("Tables is never", () => {
      type Tables = DatabaseSchema.Tables<typeof emptySchema>;
      expectTypeOf<Tables>().toEqualTypeOf<never>();
    });
  });
