...
| `OrderKey`                 | Schema and inferred type for decoded key values, including IDs and missing fields as `undefined`.                                                                                                        |
| `forKeyFields(keyFields)`  | Returns a string codec for an `OrderKey`. Decoding checks the exact ordered field names; encoding supplies the version and field names automatically. Use runtime `keyFields`, not the type-level `Key`. |
...
Effect.gen(function* () {
  const Cursor = QueryStreamCursor.forKeyFields(query.keyFields);
