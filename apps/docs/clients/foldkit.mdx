...
export const SucceededGetNotesPage = m("SucceededGetNotesPage", {
  result: Notes.pageResult,
});
export const FailedGetNotesPage = m("FailedGetNotesPage", {
...
export const update = (model: Model, message: Message): UpdateReturn =>
  Match.value(message).pipe(
    ...
    Match.tagsExhaustive({
      ...
      SucceededGetNotesPage: ({ result }) => [
        evo(model, {
          notes: Option.map(Confect.PaginatedQuery.settle(result)),
        }),
        [],
      ],
      ClickedNextPage: () => [
          ...
          notes: Option.map(
            (state) =>
              Confect.PaginatedQuery.isInvalidCursor(error)
                ? Confect.PaginatedQuery.reset(state) // held cursors are dead; back to page 1
                : Confect.PaginatedQuery.fail(state), // closes the subscription; reopen with retry
          ),
        }),
        [],
      ],
...
```ts
const items = Confect.PaginatedQuery.page(state);
const label = `Page ${Confect.PaginatedQuery.pageNum(state)}`;
