...
Add an `error` field to any `FunctionSpec` constructor. The schema is an ordinary Effect `Schema`, so a `Schema.TaggedErrorClass` class works well for a single failure mode.
...
- **`useMutation`** and **`useAction`** return `Promise<Result<A, E>>` when the ref declares an `error` schema (and `Promise<A>` otherwise).
...
Both `HttpClient` and `WebSocketClient` add the decoded error to the returned `Effect`'s error channel, alongside the existing transport error and `Schema.SchemaError`.

```ts
HttpClient.query(refs.public.notes.getOrFail, { noteId });
// Effect.Effect<Note, NoteNotFound | HttpClientError | Schema.SchemaError>
```
