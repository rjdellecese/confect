...
`paginate` uses an exclusive start and an inclusive end. For a nonterminal page, decode its `continueCursor` with `QueryStream.deserializeCursor(page.continueCursor, query.keyFields)` and pass that full runtime key as an exclusive `start` to `narrow`. Do not decode `QueryStream.END_CURSOR` as a boundary; it is an end-of-stream sentinel.
...
| `endCursor`        | Optional inclusive end boundary. Pins the page to a key range regardless of how many values it contains. `QueryStream.END_CURSOR` pins it to the end of the stream. |
...
| `continueCursor` | String identifying the next page's exclusive start. Save it unchanged; when the end is reached, the returned sentinel is `QueryStream.END_CURSOR` (`"[]"`). Never pass that sentinel as a normal `cursor`.                                                              |
...
const next =
  first.isDone || first.continueCursor === QueryStream.END_CURSOR
    ? undefined
    : yield *
      QueryStream.paginate(query, {
...
Boundary cursors are JSON strings containing a versioned envelope: `{ version: 1, keyFields: [...], key: [...] }`. `keyFields` contains the full runtime field layout in order, including implicit IDs; `key` contains the corresponding encoded values. For the example note `n3`, the serialized cursor string contains this JSON object (using the example's stand-in ID):
...
| API                                     | Contract                                                                                                                                                             |
| --------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `serializeCursor(key, keyFields)`       | Serializes a full runtime `OrderKey` with its runtime field names. Both arguments are required; use the stream's `keyFields`, not its type-level `Key`.              |
| `deserializeCursor(cursor, keyFields?)` | Validates the envelope and returns a decoded runtime `OrderKey`, preserving IDs. If expected `keyFields` are supplied, also validates the exact ordered field names. |
