...
```ts
import { HttpClient } from "@confect/server";
import { Effect } from "effect";

const HttpClientLive = HttpClient.layer(process.env.CONVEX_URL!);
```
...
const program = Effect.gen(function* () {
  ...
  const result = yield* client.action(
...
Effect.gen(function* () {
  ...
  const identity = yield* client.query(
