...
Each Effect HTTP API will be accompanied by its own live OpenAPI documentation page, powered by [Scalar](https://github.com/scalar/scalar). By default, this is served at that HTTP API's `/docs` subpath, but this can be configured by passing a custom `apiDocsPath`.
