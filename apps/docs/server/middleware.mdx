...
export default GroupSpec.make()
  ...
  .addFunction(
    ...
    }).middleware(RequireRole, { roles: { editor: true, admin: true } }),
  )
  .addFunction(
    FunctionSpec.publicMutation({
      name: "deleteAll",
...
For a group-wide role policy, use `GroupSpec.make().middleware(RequireUser).middleware(RequireRole, { roles: { admin: true } })`; it covers every function in that group.
...
export default MiddlewareImpl.make(
  ...
  (effect, { options }) =>
    Effect.gen(function* () {
      ...
      if (options.roles[user.role] !== true) {
