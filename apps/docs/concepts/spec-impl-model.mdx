...
Your Confect API is broken up into two parts: a **spec** and its **implementation** (impl).
...
Specs are built using `@confect/core`, which means they can be shared between the server and client. This separation is what enables end-to-end type safety: the client knows the exact shape of every function's arguments and return value without importing any server code.
