...
Write standard Convex functions using the Convex API. Place them in your `confect/` directory — a common convention is a `native/` subfolder.

```ts confect/native/workpool.ts
import {
  type WorkId,
  Workpool,
  vWorkId,
...
For native Convex functions, the impl is the Convex function value itself. Pass each one directly to `FunctionImpl.make`.

```ts confect/impl/workpool.ts
...
### Call native functions from the client

Native Convex functions are available through refs just like Confect functions.
...
## Mixing Confect and native functions

A single group can contain both Confect functions (with Effect schemas) and native Convex functions. Each function's spec constructor determines how it is registered and called.

```ts confect/spec/notes.ts
import { FunctionSpec, GroupSpec } from "@confect/core";
import { Schema } from "effect";
import type { search } from "../native/search";
...
In the impl, Confect functions use Effect handlers while native functions pass through the Convex function value directly.
