...
/**
 * Shared, validator-free core for the Convex (non-Node) function builders.
 *
 * `make` accepts an optional {@link RegisteredFunction.Compilers}. When present,
 * each registered function gets Convex `args`/`returns` validators compiled from
 * its `Schema`; when omitted, the function is registered with only a `handler`
 * (Convex then accepts any args, and confect's own `Schema.decode`/`Schema.encode`
 * inside the handler still enforce correctness). This module deliberately does
 * NOT import `SchemaToValidator`, so the validator-free builder
 * (`RegisteredConvexFunctionWithoutValidators`) keeps it out of a function's
 * startup import graph; the compiling builder (`RegisteredConvexFunction`)
 * injects the compilers.
 */
export const make = (
