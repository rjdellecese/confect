...
A typical use is requiring a signed-in user with one of several permitted roles. Keep the user lookup in `RequireUser`, then let a configurable `RequireRole` consume the `CurrentUser` it provides. For this example, assume the users table has a trusted `role` field defined with `Schema.Literals(["Internal", "Buyer", "BrandPartner"])`:
...
>()("RequireRole", {
  options: () =>
    Schema.Struct({
      roles: Schema.Array(
        Schema.Literals(["Internal", "Buyer", "BrandPartner"]),
      ),
    }),
  error: () => AccessDenied,
  functionTypes: { query: true, mutation: true, action: false },
...
Attach `RequireUser` first so it supplies the loaded user, then pass the permitted roles as the second argument to `.middleware()`. One policy now covers both buyer-or-internal reads and internal-only writes:
...
For a group-wide role policy, use `GroupSpec.make().middleware(RequireUser).middleware(RequireRole, { roles: ["Internal"] })`; it covers every function in that group.
...
Provide this implementation once in the group's impl layer, alongside the `RequireUser` implementation and the function implementations. The role list is specific to each attachment, not to the registered implementation. A user matching any role in one list is allowed; an empty list denies everyone. Missing users fail with `NotSignedIn` from `RequireUser`, while signed-in users outside the permitted roles fail with `AccessDenied`.
...
Both `MiddlewareImpl.make` and `MiddlewareImpl.makeByFunctionType` receive `(effect, { options, invocation })`. The typed `options` field contains the attachment value; `invocation` contains `name`, `functionType`, `functionVisibility`, and decoded `args`. Middleware without an options schema receives `options: undefined` and can still access metadata with `(effect, { invocation })`. Destructure only what you need: `(effect, { options: { roles } })` reads the role list directly.
...
A middleware implementation wraps the downstream effect (any remaining middleware plus the handler). It receives `(effect, { options, invocation })`, with attachment configuration in `options` and the covered function's `name`, `functionType`, `functionVisibility`, and decoded `args` in `invocation`, and decides whether and how to run the effect:
