...
[Middleware](/v10/server/middleware) can declare error schemas too; a middleware's errors join the error union of every function it covers, alongside the function's own `error` schema.
...
See [React](/v10/clients/react) for full examples.
...
`WebSocketClient.reactiveQuery` carries typed errors too: when the subscribed
query fails, its `Stream` terminates with the decoded value in the error
channel. `reactiveQueryResult` instead emits successes and failures as `Result`
values while leaving the Convex subscription open. See
[HTTP](/v10/clients/js/http) and [WebSocket](/v10/clients/js/websocket).

### `@confect/test`

`TestConfect`'s `query`, `mutation`, and `action` decode typed errors into the `Effect` error channel, so test bridges see the same shape as the JS clients. See [Testing](/v10/guides/testing).
