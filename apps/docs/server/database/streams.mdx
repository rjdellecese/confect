...
All inputs must have the same order key, document type, and order direction, which are all type errors otherwise. The direction is `"asc"` when omitted and the given literal when passed one; a direction chosen at runtime types as `"asc" | "desc"`, so every input has to be typed that way too, and the mismatch is then caught when the stream runs. Overlapping inputs are not deduplicated: a document matched by two inputs appears twice.
