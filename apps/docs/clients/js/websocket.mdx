...
```ts
import { WebSocketClient } from "@gunta/confect-js";
...
Use the `WebSocketClient` service inside `Effect.gen` to call your functions with refs, the same way you would with `HttpClient`, `@gunta/confect-react` hooks, or `@gunta/confect-test`.
