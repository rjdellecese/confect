...
/**
 * The name of a table's generated document type in `confect/_generated/docs.ts`
 * (e.g. `UserProfilesDoc`). Branded so it can't be confused with an arbitrary
 * string—construct it with {@link fromTableName}.
 */
...
/**
 * Convert a Convex table name to the name of its generated document type in
 * `confect/_generated/docs.ts`.
 *
 ...
 * name (`user_profiles` and `userProfiles` both become `UserProfilesDoc`). That
 * folding can make two distinct tables map to the same document name; codegen
 * guards against it (see `validateNoDocNameCollisions`).
 *
