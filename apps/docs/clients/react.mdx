...
When the spec declares an `error` schema, the result type gains a `Failure` variant carrying the decoded typed error — errors are values, consistent with `useQuery`. Narrow with `PaginatedQueryResult.isFailure` (or handle `onFailure` in `match`) before accessing `results`:
...
if (PaginatedQueryResult.isFailure(notes)) {
  return <p>Failed: {notes.error.reason}</p>;
}
// notes: Loaded — results/isLoading/loadMore are available.
```
