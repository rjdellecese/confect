...
The same rules apply as at the group level, at the same authoring sites: attaching a middleware whose `kinds` don't include the function's kind, attaching to a [plain Convex function](/server/plain-convex-functions), or attaching the same middleware twice — including once at each level, in either order — are all type errors. Implementations are provided to the group's impl layer exactly like group-level ones, and `GroupImpl.finalize` demands them just the same.
...
```ts
export class RequireAdmin extends MiddlewareSpec.Service<
  RequireAdmin,
  {
...
const RequireAdminLive = MiddlewareImpl.make(
  databaseSchema,
  RequireAdmin,
  (effect) =>
    Effect.gen(function* () {
