...
An order key contains the values that determine a document's position. For a note ordered by text, it might be `["apple", 1, "n1"]`: text, creation time, and an ID tiebreaker. Pinning text with `eq` leaves `[1, "n1"]`.
