...
Implement your endpoint handlers in the same file using `HttpApiBuilder`. All Confect action [services](/v9/concepts/services) (`QueryRunner`, `MutationRunner`, `ActionRunner`, `Scheduler`, `Auth`, `StorageReader`, `StorageWriter`, `StorageActionWriter`) are available in handlers.
