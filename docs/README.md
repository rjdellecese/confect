...
Confect is a framework that deeply integrates the [Effect](https://effect.website) ecosystem with [Convex](https://convex.dev) APIs. But it's more than just Effect bindings—Confect allows you to:
...
* Use Effect-ified versions of all of the Convex server APIs (`Promise`s become `Effect`s, `A | null` becomes `Option<A>`s, etc.).
