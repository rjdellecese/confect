...
| `codecForKeyFields(keyFields)` | Returns a string codec for an `OrderKey`. Decoding checks the exact ordered field names; encoding supplies the version and field names automatically. Use runtime `keyFields`, not the type-level `Key`. |
