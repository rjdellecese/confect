...
A complete pair looks like this. The impl default-imports its sibling spec and passes it to `FunctionImpl.make` and `GroupImpl.make`:
...
<Note>
  ...
  path it produces—`confect/notesAndRandom/notes.spec.ts` becomes
  `internal.notesAndRandom.notes.list`. Renaming a file or directory
...
Place [node action](/server/node-actions) spec and impl files in a `node/` subdirectory, using the same `.spec.ts`/`.impl.ts` suffixes. Their specs use `GroupSpec.makeNode()` and their impls use the `nodeApi` from `_generated/nodeApi`.
