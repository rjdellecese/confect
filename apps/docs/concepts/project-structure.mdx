...
Contains a single default `Api` export, which combines your database schema and spec. Impl files pass the database schema — the default export of `_generated/schema.ts` — as the first argument to `GroupImpl.make` and `FunctionImpl.make`; they no longer take the `Api` itself.
...
Contains a single default `Api` export, which combines your database schema and Node spec. Like non-node impls, node impls pass the database schema (`_generated/schema.ts`'s default export) to `GroupImpl.make` and `FunctionImpl.make`.
