...
Add `ExecutionMetadata`, `RequestMetadata`, and `TransactionMetadata` services for reading function and deployment metadata, request metadata, and current transaction metrics. Generated services expose each capability to the function contexts that support it, including middleware and HTTP handlers where applicable.

```ts
import * as Effect from "effect/Effect";
import { ExecutionMetadata, TransactionMetadata } from "./_generated/services";
