...
Create the Convex HTTP router in `confect/http.ts` using `HttpApi.make` from `@gunta/confect-server`. Each key is a path prefix that must be either `"/"` or end with a trailing slash (e.g. `"/api/"`).
