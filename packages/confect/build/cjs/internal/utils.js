...
const collectBranchLeaves = (obj, leafRefinement, path = []) => {
	const leaves = effect.Record.filter(obj, leafRefinement);
	const currentBranch = effect.Record.keys(leaves).length > 0 ? [{
		path,
		values: leaves
	}] : [];
	const nestedBranches = effect.Array.flatMap(effect.Record.keys(obj), (key) => {
		const value = obj[key];
...
const forEachBranchLeaves = (obj, leafRefinement, f) => {
	...
	return effect.Effect.forEach(branchLeaves, f, { discard: true });
};
...
//#endregion
exports.forEachBranchLeaves = forEachBranchLeaves;
exports.mapLeaves = mapLeaves;
exports.setNestedProperty = setNestedProperty;
exports.validateJsIdentifier = validateJsIdentifier;
