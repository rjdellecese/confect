...
| `Transaction`       | `getMetrics()`                     | Queries and mutations                          |
...
`Transaction.getMetrics()` returns Convex's `TransactionMetrics`: the `used` and `remaining` amounts for limits such as bytes read, documents written, database queries, and scheduled functions. Read these metrics within a query or mutation to inspect its current usage.

```ts
import * as Effect from "effect/Effect";
import { Transaction } from "./_generated/services";
