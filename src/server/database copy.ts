...
type ConfectDatabaseReader_<ConfectDataModel extends GenericConfectDataModel> =
  {
    readonly get: <
      TableName extends
...
const ConfectDatabaseReader_ = <
  ConfectDataModel extends GenericConfectDataModel,
>() =>
  Context.GenericTag<ConfectDatabaseReader_<ConfectDataModel>>(
    "@rjdellecese/confect/ConfectDatabaseReader_",
  );

const ConfectDatabaseReaderLive = <
  ConfectDataModel extends GenericConfectDataModel,
>(
  lazyArgs: () => {
    db: BaseDatabaseReader<DataModelFromConfectDataModel<ConfectDataModel>>;
  ...
  const { db, databaseSchemas } = lazyArgs();

  return Layer.succeed(ConfectDatabaseReader_<ConfectDataModel>(), {
    get: (tableName, id) =>
      // If system table, use `db.system.get`
      Effect.promise(() => db.get(id)).pipe(Effect.map(Option.fromNullable)),
  });
};

type ConfectDatabaseWriter_<ConfectDataModel extends GenericConfectDataModel> =
  Expand<
    ConfectDatabaseReader_<ConfectDataModel> & {
      readonly insert: <
...
const ConfectDatabaseWriter_ = <
  ConfectDataModel extends GenericConfectDataModel,
>() =>
  Context.GenericTag<ConfectDatabaseWriter_<ConfectDataModel>>(
    "@rjdellecese/confect/ConfectDatabaseWriter_",
  );

const ConfectDatabaseWriterLive = <
  ConfectDataModel extends GenericConfectDataModel,
>(
  lazyArgs: () => {
    db: GenericDatabaseWriter<DataModelFromConfectDataModel<ConfectDataModel>>;
  ...
  return Layer.effect(
    ConfectDatabaseWriter_<ConfectDataModel>(),
    Effect.gen(function* () {
      const baseDatabaseReader =
        yield* ConfectBaseDatabaseReader_<ConfectDataModel>();

      return {
        ...baseDatabaseReader,
        insert: (table, value) => Effect.promise(() => db.insert(table, value)),
      };
    }),
  );
};
