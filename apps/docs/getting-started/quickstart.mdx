...
<Steps>
  ...
  <Step title="Run the codegen command">
    Generate your app's assembled spec (`confect/_generated/spec.ts`), API, services, and per-group registered functions.
  ...
  <Step title="Implement your Confect functions">
    ...
    Run `confect codegen` again so the CLI emits the per-group registry that wires this impl into `convex/`.
