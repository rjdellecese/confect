...
When a ref's spec declares an `error` schema, the decoded error is added to the error channel of `query`, `mutation`, and `action` alongside `ParseError`. See [Error Handling](/v10/server/error-handling) for how to declare error schemas.
...
Use `fetch` to test your [HTTP API](/v10/server/http-api) endpoints.
