...
<Warning>
  Nothing a `*.spec.ts` reaches may value-import `@confect/server`. Specs are
  reachable from `_generated/refs.ts`, which your client imports, so their whole
  import graph is bundled into the browser — a middleware implementation left in
  a spec module ships your authorization logic to users. `confect codegen` fails
  when this happens; `import type` is always fine, since it is erased before
  bundling.
</Warning>
