...
Pass the document type as a type argument, then reuse a compatible stream's `keyLayout`. Creating that stream does not read documents. The [generated document types](/v10/server/database/schema#document-types) name the document:
...
`QueryStream.filter` removes emitted values; `QueryStream.map` transforms them. Both preserve the original stored keys. A rejected document still counts as _read_, so cursors can advance past it.
