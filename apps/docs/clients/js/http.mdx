...
```ts
import { HttpClient } from "@gunta/confect-js";
...
Use the `HttpClient` service inside `Effect.gen` to call your functions with refs, the same way you would with `@gunta/confect-react` hooks or `@gunta/confect-test`.
