...
When a function declares an [`error` schema](/server/typed-errors), `query`, `mutation`, and `action` surface the decoded error in the `Effect` error channel. Use `Effect.either` or `Effect.exit` to assert on the failure path.
