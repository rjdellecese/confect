...
When the ref declares an [`error` schema](/server/error-handling), the decoded typed error is exposed in the `Effect` error channel, so you can assert against it with `Effect.either` or recover with `Effect.catchTag`:
...
Effect.gen(function* () {
  ...
  const result = yield* Effect.either(
    c.query(refs.public.notes.getOrFail, { noteId }),
  );
