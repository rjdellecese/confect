...
<Note>
  This page targets the `@confect/test@effect4` prerelease, which
  depends on `effect@4.0.0-beta.67`, `@effect/vitest@4.0.0-beta.67`,
  `convex@^1.39.0`, and `convex-test@^0.0.53`. For Effect 3 docs, see
  the stable `@confect/test` (latest) release.
</Note>
...
<Tip>
  `convex-test@^0.0.53` builds on the parallel-test isolation upgrade
  introduced in `0.0.45`: each `convexTest()` instance scopes its
  in-memory state via `AsyncLocalStorage`, which removes flakiness
  when multiple test files (or `describe` blocks) run concurrently.
</Tip>
