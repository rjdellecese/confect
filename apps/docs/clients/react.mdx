...
`@gunta/confect-react` provides drop-in replacements for Convex's React hooks. Each hook automatically encodes your args and decodes return values through the [Effect Schemas](/concepts/schema-restrictions) defined in your [function specs](/concepts/spec-impl-model). You work with the Schema `Type` (decoded) values on both sides—the hooks handle the round-trip to Convex's `Encoded` representation transparently.
...
```tsx
import { QueryResult, useQuery } from "@gunta/confect-react";
...
```tsx
import { useMutation } from "@gunta/confect-react";
import refs from "../confect/_generated/refs";
...
```tsx
import { useAction } from "@gunta/confect-react";
import refs from "../confect/_generated/refs";
